...
def plot_lagllama_predictions(_input, _target, median, low, high, country, reso, _type, _path = 'exp/lag-llama/result/'):
    plt.figure(figsize=(6, 4))
    ...
    plt.xlabel('Time [Hour]', fontsize=14)
    plt.ylabel('Electricity Consunption [kWh]', fontsize=14)
    plt.title(f'Lag-Llama Predictions for {country.capitalize()}-{_type.capitalize()}-{reso}', fontsize=16)
    ...
    # Save the plot
    os.makedirs(_path, exist_ok=True)
    plt.savefig(os.path.join(_path, f'lag_llama_{country}_{reso}_{_type}.png'), bbox_inches='tight')
    plt.close()
